...
exp_sorted = pd.DataFrame(np.sort(expl.values, axis=0), 
                            index=expl.index, 
                            columns=expl.columns)

exp_mean = exp_sorted.mean(axis=1)
exp_sorted=0
expr = expl.rank(method="min").astype(np.int)
...
rank_mean = expl.stack().groupby(expl.rank(method='first').stack().astype(int)).mean()


ee = expl.rank(method='min').stack().astype(int).map(exp_mean).unstack()
...
h5file = "mouse_matrix.h5"
f = h5.File(h5file, 'r')
expression = f['data/expression']
samples = f['meta/Sample_geo_accession']
genes = list(f['meta/genes'])
genes = [x.upper() for x in genes]

cco = pd.read_feather("correlation_folder_300/correlation_global.f")
ge = cco["index"]

gidx = [genes.index(x) for x in ge]
cc = calculateCorrelation("mouse_matrix.h5", clustering=[1,1,1], geneidx=gidx)

globalSampleCount = 5000
expression = f['data/expression']
samples = f['meta/Sample_geo_accession']
genes = f['meta/genes']
...
exp = expression[samplesidx,:][:, gidx]
